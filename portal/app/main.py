...
def ensure_schema():
    ...
    try:
        with engine.begin() as conn:
            ...
            conn.execute(text("ALTER TABLE clinic_settings ADD COLUMN IF NOT EXISTS sms_provider VARCHAR(50) DEFAULT 'infobip'"))
            conn.execute(text("ALTER TABLE clinic_settings ADD COLUMN IF NOT EXISTS infobip_username VARCHAR(200) DEFAULT ''"))
            conn.execute(text("ALTER TABLE clinic_settings ADD COLUMN IF NOT EXISTS infobip_userkey VARCHAR(300) DEFAULT ''"))

            # app_license: activation fields
            conn.execute(text("ALTER TABLE app_license ADD COLUMN IF NOT EXISTS client_id VARCHAR(120) DEFAULT ''"))
            conn.execute(text("ALTER TABLE app_license ADD COLUMN IF NOT EXISTS activation_token VARCHAR(2000) DEFAULT ''"))
...
def seed_if_empty():
    ...
    try:
        ...
        if not db.get(AppLicense, 1):
            ...
            db.add(AppLicense(id=1, product_mode="BOTH", trial_end=datetime.utcnow() + timedelta(weeks=4)))
            db.commit()
        ...
        appt = Appointment(child_id=child1.id, starts_at=now + timedelta(days=1, hours=10), ends_at=now + timedelta(days=1, hours=11),
                           therapist_name="", procedure="Session", attendance_status="scheduled")
        db.add(appt)
        db.commit()
    finally:
        db.close()
